class ssh::server(
  ...
  $hiera_options = lookup("${module_name}::server::options", {merge => hash, default_value => {}})
  $hiera_match_block = lookup("${module_name}::server::match_block", {merge => hash, default_value => {}})
